	...
	def is_complete(self):
		...
		for vertex in self.vertices():
			...
			if vertex in self.__graph_dict[vertex]:
				...
				if len(self.__graph_dict[vertex]) < len(self.__graph_dict) - 1:
					return False

		return True

	def is_subgraph(self,F):
		"""checks if a graph F is a subgraph of this graph"""
		for vertex in F.vertices():
			if vertex not in self.__graph_dict.keys():
				return False
			
			for connected_vertex in F.__graph_dict[vertex]:
				if connected_vertex not in self.__graph_dict[vertex]:
					return False
...
if __name__ == "__main__":
	...
	print graph.is_subgraph(graph2)
